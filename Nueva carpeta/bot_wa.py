...
def _ws_sesiones(sheet):
    ...
    try:
        ...
        # Verificar y ampliar columnas si es necesario
        props = ws.spreadsheet.fetch_sheet_metadata()
        for s in props.get("sheets", []):
            if s["properties"]["title"] == "Sesiones":
                cols = s["properties"]["gridProperties"]["colCount"]
                rows = s["properties"]["gridProperties"]["rowCount"]
                if cols < SESIONES_COLS or rows < SESIONES_ROWS:
                    body = {"requests": [{"updateSheetProperties": {
                        "properties": {
                            "sheetId": s["properties"]["sheetId"],
                            "gridProperties": {
                                "rowCount": max(rows, SESIONES_ROWS),
                                "columnCount": max(cols, SESIONES_COLS)
                            }
                        },
                        "fields": "gridProperties"
                    }}]}
                    sheet.spreadsheet.batch_update(body)
                    log.info(f"✅ Sesiones ampliada a {SESIONES_ROWS}x{SESIONES_COLS}")
        return ws
    except Exception as e:
        if "Sesiones" in str(e) or "not found" in str(e).lower() or "not exist" in str(e).lower():
            ws = sheet.spreadsheet.add_worksheet(
                title="Sesiones", rows=SESIONES_ROWS, cols=SESIONES_COLS)
            ws.append_row(SESIONES_HDR)
            log.info("✅ Hoja Sesiones creada")
            return ws
        # Si falla el resize también, intentar crear de cero
        try:
            ws = sheet.spreadsheet.add_worksheet(
                title="Sesiones", rows=SESIONES_ROWS, cols=SESIONES_COLS)
            ws.append_row(SESIONES_HDR)
            ...
            log.error(f"❌ No se pudo crear/ampliar hoja Sesiones: {e2}")
            raise
...
def _save(sess, ws):
    ...
    # Actualizar celda por celda para mayor compatibilidad
...
def _reset(sess, ws):
    r = sess["row_ws"]
    try:
        for col in range(2, SESIONES_COLS + 1):
