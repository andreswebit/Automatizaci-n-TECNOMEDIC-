...
# Palabras que siempre muestran el menú (sin importar el step)
_PALABRAS_MENU = {
...
# Palabras que siempre despiden
_PALABRAS_SALIR = {
    "5", "salir", "exit", "chau", "bye", "adios", "adiós",
    "gracias", "ok gracias", "listo", "no gracias"
}
...
def _get_ocupados(sheet):
    ...
    try:
        ...
        h = rows[0]
        i_f = h.index("Fecha")  if "Fecha"  in h else 7
        i_h = h.index("Hora")   if "Hora"   in h else 8
        i_e = h.index("Estado") if "Estado" in h else 9
        for r in rows[1:]:
            if len(r) <= max(i_f, i_h, i_e): continue
            if r[i_e].strip().lower() == "cancelado": continue
            f  = r[i_f].strip()
            hh = r[i_h].strip()
            if f: ocupados.setdefault(f, {}).update(
                {hh: ocupados.get(f, {}).get(hh, 0) + 1})
    except Exception as e:
...
def _buscar_turno(sheet, nombre):
    try:
        ...
        for i, r in enumerate(rows[1:], 2):
            ...
            t = dict(zip(h, r))
            nombre_hoja = f"{t.get('Nombre','')} {t.get('Apellido','')}".strip()
            if (nombre.lower() in nombre_hoja.lower()
...
def procesar(phone, msg, sheet):
    ...
    # ── SALIR: cualquier palabra de despedida ─────────────────
    if low in _PALABRAS_SALIR:
        _reset(sess, ws)
        _enviar(phone, DESPEDIDA)
        return

    # ── MENÚ: cualquier saludo o "0" o "menu" ────────────────
    # También activa el menú si es el PRIMER mensaje (step=menu y no es número)
    if low in _PALABRAS_MENU:
        _reset(sess, ws)
        _enviar(phone, MENU)
        return

    # Si el usuario está en menú y manda algo que no es 1-4
    # → mostrar menú (cualquier texto activa el bot)
    if step == "menu":
        if txt in ("1", "2", "3", "4"):
            pass  # procesar normalmente abajo
        else:
    ...
    if step == "nuevo_nombre":
        ...
        try:
            ...
            _enviar(phone, "❌ Error al consultar agenda. Intentá de nuevo o escribí *0*.")
            return
        if not disp:
    ...
    if step == "nuevo_hora":
        ...
        sess["hora"] = hora_elegida
        sess["step"] = "nuevo_email"
        _save(sess, ws)
        _enviar(phone,
            f"✅ *{sess['fecha']}* a las *{hora_elegida}hs*\n\n"
            f"Ingresá tu *email* para recibir la confirmación:"
        )
        return
    ...
    if step == "nuevo_email":
        ...
        partes   = nombre.split()
        nom      = partes[0] if partes else nombre
        apellido = " ".join(partes[1:]) if len(partes) > 1 else ""

        try:
            sheet.append_row([nom, apellido, "", "", "", tel, email, fecha, hora, "Pendiente"])
        ...
        _enviar(phone,
            ...
            f"📞 (3794) 34-9278\n\n"
            f"Escribí *5* para salir o *0* para el menú 😊"
        )
        _reset(sess, ws)
        return
    ...
    if step == "mod_nombre":
        ...
        sess["step"] = "mod_fecha"
        _save(sess, ws)
        nombre_t = f"{t.get('Nombre','')} {t.get('Apellido','')}".strip()
        _enviar(phone,
            f"📋 *Turno actual:*\n"
            f"👤 {nombre_t}\n"
    ...
    if step == "mod_hora":
        ...
        try:
            # Columnas nuevas: Fecha=8, Hora=9, Estado=10
            sheet.update_cell(fila, 8, sess["fecha"])
            sheet.update_cell(fila, 9, hora_elegida)
            sheet.update_cell(fila, 10, "Pendiente")
        except Exception as e:
            log.error(f"Error modificando Sheets: {e}")
            _enviar(phone, "❌ Error al modificar el turno. Intentá de nuevo."); return
        _enviar(phone,
    ...
    if step == "cancel_conf":
        if low in ("si", "sí", "s", "yes"):
            fila = sess.get("fila_turno", 0)
            try:
                sheet.update_cell(fila, 10, "Cancelado")   # columna Estado = 10
            except Exception as e:
                log.error(f"Error cancelando: {e}")
                _enviar(phone, "❌ Error al cancelar. Llamanos al (3794) 34-9278."); return
            _enviar(phone,
                "✅ Turno *cancelado*.\n\n"
                "Escribí *1* para sacar un nuevo turno\n"
                "o *5* para salir."
            )
        else:
            _enviar(phone,
                "👍 Cancelación abortada. Tu turno sigue activo.\n\n"
                "Escribí *0* para el menú o *5* para salir."
            )
        _reset(sess, ws); return

    # ── Step desconocido → mostrar menú ──────────────────────
